...
const EditKutuModal = ({ isOpen, onClose, onSubmit, item }) => {
  ...
  useEffect(() => {
    if (item) {
      ...
      setFormData({
        ...
        boy: dims[1] || 0,
        yukseklik: dims[2] || 0,
        icine_giren_urun: item.icine_giren_urun || '',
        stock: item.stock || 0
      });
    }
  }, [item]);
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({
      name: formData.name,
      dimensions,
      icine_giren_urun: formData.icine_giren_urun,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-800">Kutu Düzenle</h2>
          <button
            onClick={onClose}
