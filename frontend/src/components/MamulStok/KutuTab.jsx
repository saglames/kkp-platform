...
import EditKutuModal from './EditKutuModal';
...
const KutuTab = () => {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const data = await mamulStokAPI.getKutu();
      setItems(data);
    } catch (error) {
  ...
  const handleQuickStockChange = async (item, action) => {
    ...
    try {
      await mamulStokAPI.changeKutuStock(item.id, {
  ...
  const handleModalSubmit = async (data) => {
    try {
      await mamulStokAPI.changeKutuStock(selectedItem.id, data);
      setShowModal(false);
  ...
  const handleAddSubmit = async (data) => {
    try {
      await mamulStokAPI.addKutu(data);
      setShowAddModal(false);
      fetchItems();
      alert('Yeni kutu başarıyla eklendi!');
    } catch (error) {
  ...
  const handleEditSubmit = async (data) => {
    try {
      await mamulStokAPI.updateKutu(editItem.id, data);
      setEditModalOpen(false);
      setEditItem(null);
      fetchItems();
      alert('Kutu başarıyla güncellendi!');
    } catch (error) {
  ...
  const handleConfirmDelete = async () => {
    try {
      await mamulStokAPI.deleteKutu(deleteItem.id);
      setConfirmModalOpen(false);
      setDeleteItem(null);
      fetchItems();
      alert('Kutu başarıyla silindi!');
    } catch (error) {
  ...
  const filteredItems = items.filter(item => {
    ...
    return (
      ...
      (item.icine_giren_urun && item.icine_giren_urun.toLowerCase().includes(search))
    );
  });
    ...
    <div>
      <div className="mb-6 flex justify-between items-center gap-4">
        <div className="flex-1 max-w-md">
          <input
            type="text"
            placeholder="Kutu ara (ad, ölçü, içindeki ürün)..."
            value={searchTerm}
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">İçine Giren Ürün</th>
              <th
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {sortedAndFilteredItems.map((item) => (
              <tr key={item.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-sm text-gray-600">{item.icine_giren_urun || '-'}</td>
      ...
      {sortedAndFilteredItems.length === 0 && (
        <div className="text-center py-12 text-gray-500">
          {searchTerm ? 'Arama sonucu bulunamadı.' : 'Henüz kutu eklenmemiş.'}
        </div>
      )}
      ...
      {confirmModalOpen && deleteItem && (
        <ConfirmModal
          ...
          title="Kutu Sil"
          message={`"${deleteItem.name}" adlı kutuyu silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.`}
        />
      )}
    </div>
  );
