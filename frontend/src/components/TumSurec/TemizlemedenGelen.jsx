...
const TemizlemedenGelen = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const data = await tumSurecAPI.getTemizlemedenGelen();
      setUrunler(data);
    } catch (error) {
  ...
  const handleSevkeHazirlaClick = (urun) => {
  ...
  const handleTransfer = async () => {
    ...
    if (adet > selectedUrun.adet) {
      alert(`Maksimum ${selectedUrun.adet} adet sevke hazırlanabilir!`);
      return;
    }

    try {
      const response = await tumSurecAPI.sevkeHazirla({
      ...
      alert(response.message || 'Ürün sevke hazır duruma getirildi!');
      setShowModal(false);
      fetchData();
    } catch (error) {
  ...
  const handleSubmitAdd = async (e) => {
    ...
    try {
      await tumSurecAPI.addTemizlemedenGelen(formData);
      alert('Ürün eklendi!');
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await tumSurecAPI.deleteTemizlemedenGelen(id);
      alert('Ürün silindi!');
      fetchData();
    } catch (error) {
  ...
  return (
    <div>
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Temizlemeden Gelen Ürünler</h2>
          <p className="text-gray-600 mt-1">Temizlik süreci tamamlanmış ürünler</p>
        </div>
            ...
            {urunler.length === 0 ? (
              <tr>
                <td colSpan="6" className="px-4 py-8 text-center text-gray-500 text-lg">
                  Temizlemeden gelen ürün yok
                </td>
              </tr>
            ) : (
              urunler.map((urun) => (
                <tr key={urun.id} className="border-b hover:bg-purple-50 transition-colors">
                  ...
                  <td className="px-4 py-4 text-center text-lg font-bold text-purple-600 border-r">
                    {urun.adet}
                  </td>
                  ...
                  <td className="px-4 py-4 text-center">
                    <div className="flex gap-2 justify-center">
                      ...
                      <button
                        onClick={() => handleSevkeHazirlaClick(urun)}
      ...
      {showModal && selectedUrun && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4">
            <h3 className="text-2xl font-bold mb-6 text-gray-800">Sevke Hazırla</h3>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={handleEdit}
                className="flex-1 px-6 py-3 bg-purple-500 text-white text-lg font-medium rounded-lg hover:bg-purple-600 transition-colors"
              >
                Kaydet
              </button>
            </div>
...
export default TemizlemedenGelen;
